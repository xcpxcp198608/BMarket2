...
public class DownloadTask implements Runnable {
    ...
    public static final int START_DOWNLOAD_FILE = 0X10001;
    public static final int PAUSE_DOWNLOAD_FILE = 0X10002;
    public static final int COMPLETED_DOWNLOAD_FILE = 0X10003;
    public static final int PROGRESS_CHANGE_DOWNLOAD_FILE = 0X10004;
    private Context context;

    public DownloadTask(Context context , DownloadFileInfo downloadFileInfo ,String filePath) {
        ...
        try {
            ...
            if(httpURLConnection.getResponseCode() == HttpURLConnection.HTTP_OK){
                ...
                if(downloadFileInfo.getFileLength() < 0){
                    ...
                    handler.obtainMessage(START_DOWNLOAD_FILE , downloadFileInfo.getFileLength()).sendToTarget();
                }
            }
            ...
            if(httpURLConnection1.getResponseCode() == HttpURLConnection.HTTP_PARTIAL){
                ...
                while ((length = inputStream.read(buffer)) != -1) {
                    ...
                    if(System.currentTimeMillis() - time > 1000) {
                        ...
                        Log.d("----px----",downloadFileInfo.getFileFullName()+"--"+progress);
                        handler.obtainMessage(PROGRESS_CHANGE_DOWNLOAD_FILE ,progress).sendToTarget();
                    }
                    if(isPause){
                        ...
                        handler.obtainMessage(PAUSE_DOWNLOAD_FILE , (int)(completedPosition*100L/downloadFileInfo.getFileLength())).sendToTarget();
                        return;
                    }
                }
                handler.obtainMessage(COMPLETED_DOWNLOAD_FILE).sendToTarget();
                if(sqLiteDao.isExists(downloadFileInfo)){
